...
each token page carries `noindex`; CI enforces both and probes the live domain. Access
logs redact the credential tail of token paths. *Tests: `test_seo_privacy.py`;
...
owner-only. Culled/hidden frames never resurface to a client. Uploads are size-bounded
(75 MB/image, bounded read) so one studio can't OOM the box. *Tests:
...
constant-time master-token compare; owner sessions can't reach admin. *Tests:
`test_auth_kdf.py`.*
...
login, signup, reset, inquiry, booking, and checkout.
