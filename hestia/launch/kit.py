"""Launch kit read model — operator aggregates, cohort views, and runbook."""
...
def beta_launch_kit(conn: sqlite3.Connection, settings: Settings) -> dict:
    ...
    nudge_activity = launch_nudge_activity(conn)
...
def beta_launch_export_rows(conn: sqlite3.Connection, settings: Settings) -> list[dict]:
    ...
    for studio in cockpit["studios"]:
        row_followup = followups.get(studio["tenant_id"]) or followup(
            studio,
            nudge_activity=nudge_activity,
        )
        rows.append({
            ...
            "followup_prompt": row_followup["prompt"],
            "mailto": row_followup["mailto"],
            "last_nudged_at": row_followup["last_nudged_at"],
            "nudge_status": row_followup["nudge_status"],
        })
    return rows


def launch_nudge_activity(
...
def launch_digest_recipient(settings: Settings) -> str:
    return (settings.smtp_from or settings.smtp_user or "").strip()


def launch_digest_recent(conn: sqlite3.Connection, *, cooldown_days: int) -> bool:
    row = conn.execute(
        """
        SELECT 1
          FROM audit_log
...
def _revenue_pipeline(
    ...
    *,
    ...
    stage_specs = [
        ...
        ("Paid", paid_count, f"{price_label(settings, paid_count)} in current flat-plan MRR.",
...
def price_label(settings: Settings, paid_count: int) -> str:
...
def _followups(
    ...
    *,
    ...
    return [
        followup(studio, nudge_activity=nudge_activity)
